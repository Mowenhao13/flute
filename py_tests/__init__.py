...
class SenderReceiverTestCase(TestCase):
    ...
    # def test_file_transmission(self):
    #     from flute import sender, receiver
    #     print("------- test_file_transmission --------")
        
    #     # 1. 读取本地文件内容
    #     with open("/home/halllo/flute-main/py_tests/file_test.txt", "rb") as f:
    #         file_data = f.read()  # 读取二进制数据

    #     # 2. 初始化发送端
    #     sender_config = sender.Config()
    #     oti = sender.Oti.new_no_code(1400, 64)  # 参数根据文件大小调整
    #     flute_sender = sender.Sender(1, oti, sender_config)

    #     # 3. 添加文件到发送端
    #     flute_sender.add_object_from_buffer(
    #         file_data, 
    #         "text/plain",                  # MIME类型
    #         "file://file_test.txt",        # 目标URI
    #         None                           # 可选元数据
    #     )
    ...
    #     flute_receiver = receiver.Receiver(udp_endpoint, 1, receiver_writer, receiver_config)
